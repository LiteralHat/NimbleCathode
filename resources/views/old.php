<!DOCTYPE html>
<html lang="en" class="overscroll-x-none">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>LiteralHat.com</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>


<body class="h-screen">

    <div class="flex items-center justify-center h-screen m-0 p-10">
        <div class="flex flex-col justify-center w-[600px] ">
            <img src="/img/logo.png" class="my-24 w-3/5 h-auto">
            <ul class="font-poppins space-y-6 text-2xl pl-6">
                <li>About</li>
                <li>Commission</li>
                <li>Portfolio</li>
                <li>Website Design</li>
                <li>Services</li>
            </ul>
        </div>

        <div>
            <img src="/img/pagedolls/guy.png" class="h-auto w-[600px] mt-96">
            <div class="w-screen h-full bg-black absolute"></div>
        </div>

    </div>




</body>

</html>